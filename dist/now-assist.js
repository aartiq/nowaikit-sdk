...
export class NowAssistClient {
     ...
     */
    async executeSkill(skillSysId, input) {
        const baseUrl = this.client.getBaseUrl();
        const response = await this.client.request(`${baseUrl}/api/now/now_assist/skills/${encodeURIComponent(skillSysId)}/execute`, {
            method: 'POST',
     ...
     */
    async listSkills(active) {
        let query = '';
        if (active !== undefined) {
            query = `?sysparm_query=active=${active}`;
        }
        const baseUrl = this.client.getBaseUrl();
        const response = await this.client.request(`${baseUrl}/api/now/now_assist/skills${query}`, { method: 'GET' });
        return response;
    }
    /**
    ...
    async createConversation(topic) {
        ...
        const response = await this.client.request(`${baseUrl}/api/now/now_assist/conversations`, {
            method: 'POST',
    ...
    async sendMessage(conversationId, message) {
        const baseUrl = this.client.getBaseUrl();
        const response = await this.client.request(`${baseUrl}/api/now/now_assist/conversations/${encodeURIComponent(conversationId)}/messages`, {
            method: 'POST',
