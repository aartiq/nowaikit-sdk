...
const logger = {
    debug(message, _data) {
        if (process.env.NOWAIKIT_DEBUG === 'true')
...
export class ServiceNowClient {
    ...
    async request(url, options = {}) {
        ...
        if (lastError) {
            ...
            if (cause) {
                throw new ServiceNowError(`Request failed: ${cause.message}`, cause.code || 'NETWORK_ERROR');
            }
            throw lastError;
        }
    ...
    async queryRecords(params) {
        ...
        if (params.orderBy) {
            if (params.orderBy.startsWith('-')) {
                ...
                queryParams.set('sysparm_query', params.query ? `${params.query}^ORDERBY${field}^ORDERBYDESC` : `ORDERBY${field}^ORDERBYDESC`);
            }
            else {
                queryParams.set('sysparm_query', params.query ? `${params.query}^ORDERBY${params.orderBy}` : `ORDERBY${params.orderBy}`);
            }
        }
    ...
    async getUser(userIdentifier) {
        ...
        if (/^[0-9a-f]{32}$/i.test(userIdentifier))
            return this.getRecord('sys_user', userIdentifier);
    ...
    async createRecord(table, data) {
        ...
        try {
            const response = await this.request(url, { method: 'POST', body: JSON.stringify(data) });
            return response.result;
        }
        catch (error) {
            if (error instanceof ServiceNowError)
    ...
    async updateRecord(table, sysId, data) {
        ...
        try {
            const response = await this.request(url, { method: 'PATCH', body: JSON.stringify(data) });
            return response.result;
        }
        catch (error) {
            if (error instanceof ServiceNowError)
    ...
    async createChangeRequest(params) {
        ...
        try {
            const response = await this.request(url, { method: 'POST', body: JSON.stringify(params) });
            return response.result;
        }
        catch (error) {
            if (error instanceof ServiceNowError)
    ...
    async batch(requests) {
        ...
        try {
            ...
            if (error instanceof ServiceNowError)
                throw error;
            throw new ServiceNowError(`Batch request failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'API_ERROR');
        }
    }
    /**
    ...
    async executeScript(script, scope) {
        ...
        try {
            ...
            if (error instanceof ServiceNowError)
                throw error;
            throw new ServiceNowError(`Script execution failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'API_ERROR');
        }
    }
}
